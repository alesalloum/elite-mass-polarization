...
def get_influencer_nodes(
    G: nx.Graph, left_nodes: List, right_nodes: List, n_influencers: float
) -> Tuple[List, List]:
    ...
    if not left_nodes or not right_nodes or n_influencers < 0:
        raise ValueError(
            "Invalid input: nodes lists must be non-empty and n_influencers must be non-negative."
        )
    ...
    return [node for node, _ in left_influencers], [
        node for node, _ in right_influencers
    ]


def rwc_perform_random_walk(
    G: nx.Graph,
    left_influencers: Set[Any],
    right_influencers: Set[Any],
    starting_node: Any,
) -> str:
...
def RWC_polarization(
    G: nx.Graph, ms: Dict[Any, int], n_influencers: float, n_sim: int, n_walks: int
) -> float:
    ...
    """
    left_nodes, right_nodes = [node for node in ms if ms[node] == 0], [
        node for node in ms if ms[node] == 1
    ]

    left_influencers, right_influencers = get_influencer_nodes(
        G, left_nodes, right_nodes, n_influencers
    )
    ...
    for _ in range(n_sim):
        ...
        for _ in range(n_walks):
            ...
            which_random_starting_node = random.choice(
                left_nodes if starting_side == "left" else right_nodes
            )
            ending_side = rwc_perform_random_walk(
                G, left_influencers, right_influencers, which_random_starting_node
            )
        ...
        e1, e2 = left_left / max(1, left_left + right_left), right_left / max(
            1, left_left + right_left
        )
        e3, e4 = left_right / max(1, right_right + left_right), right_right / max(
            1, right_right + left_right
        )
...
def AEI_polarization(G: nx.Graph, ms: Dict[Any, int]) -> float:
    ...
    c_ab = sum(
        1
        for s, t in G.edges()
        if (s in block_a and t in block_b) or (s in block_b and t in block_a)
    )
